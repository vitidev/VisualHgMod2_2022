...
namespace HGLib.Test
{
    class Utilities
    {
        ...
        public static bool ExtractZipResource( string destinationPath, Type type, string resourceName )
        {
            DirectoryInfo info = Directory.CreateDirectory( destinationPath );
            if (info != null)
            {
                ...
                if (zipStream == null)
                    zipStream = type.Assembly.GetManifestResourceStream(type.Namespace + "." + resourceName);
